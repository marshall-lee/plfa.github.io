...
<-trans-revisited : ∀ (m n p : ℕ)
  ...
  → n < p
    -----
  → m < p
<-trans-revisited m n p m<n n<p = ≤→< m p (≤-trans (<→≤ m n m<n) (≤-trans (+-monoˡ-≤ 0 1 n z≤n) (<→≤ n p n<p)))
```
...
o+o≡e : ∀ {m n : ℕ}
  ...
  → odd n
    ------------
  → even (m + n)
o+o≡e {suc m} {suc n} (suc em) (suc en) rewrite +-suc (suc m) n = suc (suc (e+e≡e em en))
```
